...
function WSSecurityCert(privatePEM, publicP12PEM, password, encoding) {
  ...
  this.signer.keyInfoProvider.getKey = function() {
    return _this.publicP12PEM;
  };
...
WSSecurityCert.prototype.validateSignature = function (xml) {

  var doc = new dom().parseFromString(xml);
  var signature = select(doc, "/*/*/*[local-name(.)='Signature' and namespace-uri(.)='http://www.w3.org/2000/09/xmldsig#']")[0];
  this.signer.loadSignature(signature.toString());
  var res = this.signer.checkSignature(xml);
  if (!res) {
    throw new Error("Error(s) in signature validation: " + this.signer.validationErrors.join(","));
  }
